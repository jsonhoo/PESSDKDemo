 ...
 * Author:  howard gong
 * Create:  2016/11/16 14:05
 */


public class LogSDCardUtil {

    public static final String TAG= LogSDCardUtil.class.getSimpleName();

    /**
     * 获取根目录，如果SD卡可用，则取SD卡私有目录，否则获取应用内部存储目录
     * @param context
     * @return
     */
    public static String getDefaultRootPath(Context context){
        ...
        if(isSdCardAvailable()){
            return LogSDCardUtil.getPersonalSDCradPath(context);
        }else{
            return getDataPath();
        }
    }
     ...
     * @return
     */
    public static String getPersonalSDCradPath(Context context){
        try{
            if (context!=null){
